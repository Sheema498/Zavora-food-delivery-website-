...
export class TripSlipGenerator {
  public static generateTripSlip(order: Order): TripSlipData {
    ...
    return {
      ...
      driverName: order.deliveryPartner?.user?.name || 'Courier Partner',
      driverPhone: order.deliveryPartner?.user?.phone || '+91 9123456789',
      vehicleType: order.deliveryPartner?.vehicleType || 'MOTORBIKE',
      vehicleNumber: order.deliveryPartner?.vehicleNumber || 'KA-01-EQ-4421',
